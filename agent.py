...
class Agent():
    ...
    def train(self, render=False, total_steps=100_000, index=1):
        ...
        rewards_per_episode = []
        epsilon_history = []
        value_function_history = []  # Track value function estimates
        ...
        for step in range(total_steps):
            ...
            if step % self.eval_freq == 0:
                ...
                # Estimate value function for 1000 steps
                avg_value = self.estimate_value_function(policy_dqn, eval_steps=1000)
                
                # Calculate mean reward over last 100 episodes
                mean_reward = np.mean(rewards_per_episode[-100:]) if rewards_per_episode else 0.0
                
                # Store value function data for plotting
                value_function_history.append(avg_value)
                eval_steps.append(step)
                
                # Log to TensorBoard
                writer.add_scalar('evaluation/avg_value_function', avg_value, step)
